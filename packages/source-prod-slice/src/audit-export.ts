 ...
 */
export function exportAudit(args: AuditExportArgs): AuditExportResult {
  const {
    report,
    decision,
    dataset,
    format = 'json',
    includeJudgeMatrix = true,
    includeMetrics = true,
    includeDataset = true,
  } = args;
 ...
 */
export function exportAuditBatch(
  reports: ReadonlyArray<{ report: PromotionReport; decision: PromotionDecision; dataset?: FrozenDataset }>,
 ...
 */
export function exportSummary(report: PromotionReport, decision: PromotionDecision): string {
  const lines = [
  ...
  for (const [ref, metric] of Object.entries(report.candidate_metrics)) {
    lines.push(
      `  ${ref}: delta=${metric.aggregate_delta.toFixed(4)}, cost_delta=${metric.cost_delta.toFixed(4)}, latency_delta=${metric.latency_delta.toFixed(4)}`
    );
  }
