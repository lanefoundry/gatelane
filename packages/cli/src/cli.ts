 ...
 *
 * Commands:
 *   gatelane init      — bootstrap a gatelane.yaml config
...
Usage:
  gatelane init [options]                   Bootstrap a gatelane.yaml config
    --url <endpoint>                       Target endpoint URL
    --queries <path>                       Text file with one query per line
    --request-template <json>              Request body template (default: {"query":"{{input}}"})
    --response-path <path>                 JSON path to extract response (e.g. "answer")
    --output <path>                        Output file (default: gatelane.yaml)
...
async function cmdInit(argv: ReadonlyArray<string>): Promise<number> {
  const { multi } = parseArgs(argv);
  const url = multi['url']?.[0] ?? 'http://localhost:8787/ai/ask';
  const queriesPath = multi['queries']?.[0];
  const requestTemplateStr = multi['request-template']?.[0];
  const responsePath = multi['response-path']?.[0];
  const outputPath = multi['output']?.[0] ?? 'gatelane.yaml';

  const requestObj = requestTemplateStr
    ? JSON.parse(requestTemplateStr)
    : { query: '{{input}}' };

  let testEntries: string[] = [];
  if (queriesPath) {
    const content = await readFile(queriesPath, 'utf-8');
    testEntries = content
      .split('\n')
      .map((l) => l.trim())
  ...
  const requestYaml = Object.entries(requestObj)
    .map(([k, v]) => `    ${k}: ${JSON.stringify(v)}`)
    .join('\n');

  let testsYaml: string;
  if (testEntries.length > 0) {
    testsYaml = testEntries
      .map(
        (q) =>
          `  - input: ${JSON.stringify(q)}\n    assert:\n      - type: llm-rubric\n        value: "回答應完整、準確且與問題相關"`,
      )
      .join('\n');
  } else {
    testsYaml = `  - input: "你的第一個測試問題"
    assert:
      - type: llm-rubric
        value: "回答應完整、準確且與問題相關"
  - input: "你的第二個測試問題"
  ...
  const configYaml = `# Generated by gatelane init
# Usage:
...
target:
  url: ${url}
  request:
${requestYaml}${responsePath ? `\n  response_path: ${responsePath}` : ''}

judge:
  provider: mock  # change to openai/anthropic for real scoring
  # model: gpt-4o
...
${queriesPath ? '' : '\n# queries_file: queries.txt  # or load from a text file (one query per line)\n'}`;

  await writeFile(outputPath, configYaml, 'utf-8');

  process.stdout.write(`created ${outputPath} with ${testEntries.length || 2} test(s)\n`);
  process.stdout.write(`next: gatelane eval --tag v1\n`);
  return 0;
}
...
async function cmdCompare(argv: ReadonlyArray<string>): Promise<number> {
  ...
  const fmtDelta = (d: number) => (d > 0 ? `+${d.toFixed(3)}` : d.toFixed(3));
  const truncate = (s: string, len: number) => s.length > len ? s.slice(0, len) + '...' : s;

  w(`\n── comparison: ${baselineTag} vs ${candidateTag} ──\n`);
  w(`matched pairs: ${report.totalPairs}\n`);
  w(`  improved:    ${report.improvements} (${pct(report.summary.pctImproved)})\n`);
  w(`  regressed:   ${report.regressions} (${pct(report.summary.pctRegressed)})\n`);
  w(`  unchanged:   ${report.unchanged}\n`);

  // Score deltas with avg + median
  const scoreNames = Object.keys(report.summary.avgScoreDelta);
  if (scoreNames.length > 0) {
    w(`\nscores:\n`);
    for (const name of scoreNames) {
      const avg = report.summary.avgScoreDelta[name] ?? 0;
      const med = report.summary.medianScoreDelta[name] ?? 0;
      w(`  ${name}:  avg Δ ${fmtDelta(avg)}  |  median Δ ${fmtDelta(med)}\n`);
    }
  }

  w(`  latency: avg Δ ${report.summary.avgLatencyDelta >= 0 ? '+' : ''}${report.summary.avgLatencyDelta.toFixed(0)}ms\n`);

  // Worst regression
  if (report.summary.worstRegression) {
    const wr = report.summary.worstRegression;
    w(`\nworst regression:\n`);
    w(`  "${truncate(wr.input, 60)}" — ${wr.scoreName} Δ ${wr.delta.toFixed(3)}\n`);
  }

  // Per-query detail
  const showAll = report.pairs.length <= 20;
  const pairsToShow = showAll
    ? report.pairs
    : [
        ...report.pairs.filter((p) => p.status === 'regressed'),
        ...report.pairs
          .filter((p) => p.status === 'improved')
          .sort((a, b) => {
            const aMax = Math.max(...Object.values(a.delta.scores));
            const bMax = Math.max(...Object.values(b.delta.scores));
            return bMax - aMax;
          })
          .slice(0, 3),
      ];
  ...
  if (pairsToShow.length > 0) {
    w(`\n── per-query detail${showAll ? '' : ' (regressions + top 3 improvements)'} ──\n`);
    for (const pair of pairsToShow) {
      const inputStr = typeof pair.input === 'string' ? pair.input : JSON.stringify(pair.input);
      const icon = pair.status === 'regressed' ? '✗' : pair.status === 'improved' ? '✓' : '─';
      w(`\n  ${icon} "${truncate(inputStr, 60)}"\n`);
      w(`    ${baselineTag}: ${truncate(pair.baselineResponse, 120)}\n`);
      w(`    ${candidateTag}: ${truncate(pair.candidateResponse, 120)}\n`);

      for (const [name, delta] of Object.entries(pair.delta.scores)) {
        const baseScore = pair.baseline.scores?.[name] ?? 0;
        const candScore = pair.candidate.scores?.[name] ?? 0;
        const marker = delta < 0 ? '✗ REGRESSION' : delta > 0 ? '✓' : '─';
        w(`    ${name}: ${baseScore.toFixed(2)} → ${candScore.toFixed(2)} (${fmtDelta(delta)}) ${marker}\n`);
      }

      const baseLat = pair.baseline.endTime
        ? new Date(pair.baseline.endTime).getTime() - new Date(pair.baseline.startTime).getTime()
        : 0;
      const candLat = pair.candidate.endTime
        ? new Date(pair.candidate.endTime).getTime() - new Date(pair.candidate.startTime).getTime()
        : 0;
      const latMarker = pair.delta.latencyMs > 1000 ? '⚠' : pair.delta.latencyMs < -100 ? '✓' : '─';
      w(`    latency: ${baseLat}ms → ${candLat}ms (${pair.delta.latencyMs >= 0 ? '+' : ''}${pair.delta.latencyMs}ms) ${latMarker}\n`);
    }
  }
...
async function main(argv: ReadonlyArray<string>): Promise<number> {
  ...
  if (command === 'init') return cmdInit(argv.slice(1));
